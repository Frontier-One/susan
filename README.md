...
- `/susan weekly status …` — Now ends with a computed **ENGINEERING — WEEK n** block (team-level only): Shipped, Intent→Prod, Change Failure, Rework, Agent-executed, **Human Attention / Change**, Review latency, First-pass Agent Success and Flow — each with a week-over-week delta (`pp` for percentages, relative `%` for counts and durations) and a ⚠️ on an adverse move. AI Cost / Shipped Change is printed as *not instrumented* rather than omitted. The model adds three interpretation lines under it (*What changed / Where we're losing time / Action*) and is forbidden from restating any figure. Every figure is arithmetic in `app/engineering_metrics.py` and is **appended after** generation, never handed to the model. Trends come from a stored snapshot (`metric_snapshots`), not from the model's memory. Deliberately no per-person breakdown — see the module docstring for the DORA sourcing. — Structured update (*workstreams* with *1. Last week* / *2. Next steps* and links) from **Slack messages**, **channel bookmarks**, **Google Drive**, and **all `GITHUB_REPOS`** in tech channels. Publishes the full report to a **Slack Canvas** and posts a short link in the channel (needs `canvases:write` + `files:read`; falls back to long channel messages if Canvas is unavailable). Optional `--no-approval` (restrict with `SUSAN_WEEKLY_AUTO_POST_USER_IDS`).
- `/susan status page …` (also `environment status`, `platform status`, `estate status`, `infra status` — note plain `status` is the *weekly* update) — The **environment status page**, rebuilt from last night's cloud-infra probe (`environment-status-page` artifact of `env-nightly`), the alert channels and the open roadmap issues. Numbers and bars are computed from the probe's `status.json`; the prose is written by **our own (sovereign-route) model** as strict JSON and HTML-escaped. Susan stores and hosts it at `GET /status/{slug}` (and `/status/latest`) behind `SUSAN_STATUS_PAGE_TOKEN`, and posts the tokened link. Schedule: `/susan schedule add status page every monday at 09:00 in #team-tech`.
...
| `SUSAN_ACTION_MODELS` | Optional | Per-task self-hosted model, `action=model` pairs — e.g. `weekly_status=glm-5.3-flash`. Long-context synthesis reads better on GLM than the default; the short digests do not need it. |
| `SUSAN_ACTION_MODEL_BASE_URLS` | Optional | The endpoint that serves the model above, `action=url` pairs. Usually required *with* it: our models sit behind per-model gateways, so naming a model the default endpoint does not serve is a 404, not a fallback. |
